    ...
    <Dialog open={true} onClose={onDismiss}>
      <DialogTitle>
        ...
        <Button
          onClick={() => {
            window.open(
              "https://github.com/TheBigSasha/sanity_config_creator/blob/main/gh_integration.md",
              "_blank",
            );
          }}
          variant={"text"}
      ...
      <DialogContent>
        ...
        <TextField
          ...
          value={username}
          helperText={
            "The username of the owner of the repository IE TheBigSasha"
          }
          onChange={(e) => {
        ...
        <Button
          onClick={() => {
            if (username && reponame && branch) {
              setLoading(true);
              fetch(genrateGitHubURL(username, reponame, branch))
                .then((res) => {
                  if (res.status === 200) {
                    res.json().then((json) => {
                      onGotJSON(json);
