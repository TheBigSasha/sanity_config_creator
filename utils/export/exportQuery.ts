import {
  ...
  SanityReferenceFieldProperties,
...
export const exportQuery = (
  ...
  if (!isDefaultSanityType(schema.type)) {
    ...
    for (const field of getFields(schema.type, getTypeObjOfString)) {
      query += `${sanitizeName(field.name)}${exportQuery(
        field,
        false,
        getTypeObjOfString,
      )}, \n`;
  ...
  if (hasFields(schema)) {
    ...
    for (const ofType of arraySchema.of) {
      if (
        !isDefaultSanityType(ofType) &&
        getTypeObjOfString(ofType).name === DEFAULT_DATA.name
      ) {
        query += `_type == '${sanitizeName(
          ofType,
        )}' => { // TODO: Add selection of fields for ${ofType} },`;
      } else if (
        !isDefaultSanityType(ofType) &&
        getTypeObjOfString(ofType).name !== DEFAULT_DATA.name
      ) {
        query += `_type == '${sanitizeName(ofType)}' => ${exportQuery(
          getTypeObjOfString(ofType),
          false,
          getTypeObjOfString,
        )},`;
