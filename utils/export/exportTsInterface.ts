...
const getTSTypeName = (
  ...
  switch (type) {
    case "Array":
      return `Array<${
        arrayType
          ? arrayType.map((type) => getTSTypeName(type)).join(" | ")
          : "any"
      }>`;
    case "Block":
    ...
    case "URL":
      return "URL";
    default:
      return isDefaultSanityType(type)
        ? type.toLowerCase()
        : sanitizeName(type);
  }
};
...
export const exportTSInterface = (
  ...
  if (hasFields(schema)) {
    ...
    if (stringSchema.internalConfig?.predefined && stringSchema.options?.list) {
      outStr += `${sanitizeName(schema.name)}: ${stringSchema.options.list
        .map((listItem) => `"${listItem.value}"`)
        .join(" | ")};\n`;
    } else {
      outStr += `${sanitizeName(schema.name)}: ${getTSTypeName(
        schema.type,
      )};\n`;
    }
  } else {
