...
export const copySchemaToClipboard = (
  fp: SanityFieldProperties,
  getTypeObjOfString: (name: string) => SanityFieldProperties,
) => {
  const ts =
    MINI_CODEGEN_MESSAGE + generateSanitySchema(fp, true, getTypeObjOfString);
  navigator.clipboard.writeText(ts);
};

export const copyQueryToClipboard = (
  fp: SanityFieldProperties,
  getTypeObjOfString: (name: string) => SanityFieldProperties,
) => {
  const ts =
    MINI_CODEGEN_MESSAGE + generateSanityQueries(fp, true, getTypeObjOfString);
  navigator.clipboard.writeText(ts);
};
